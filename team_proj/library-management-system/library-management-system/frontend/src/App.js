...
api.interceptors.response.use(
  response => response,
  error => {
    if (error.response?.status === 401 && localStorage.getItem('token') === 'demo-token') {
      // Skip 401 errors in demo mode
      return Promise.reject({ ...error, skipError: true });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (credentials.username === 'admin' && credentials.password === 'password123') {
        const demoLibrarian = { id: 'demo', username: 'admin', email: 'admin@library.com' };
        localStorage.setItem('token', 'demo-token');
        localStorage.setItem('librarian', JSON.stringify(demoLibrarian));
        toast.success('Login successful (Demo Mode)!');
        onLogin(demoLibrarian);
      } else {
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      const token = localStorage.getItem('token');
      if (token === 'demo-token') {
        setStats({ totalBooks: 25, issuedBooks: 8, returnedBooks: 42, pendingApprovals: 3 });
        return;
      }
      try {
...
const Books = () => {
  ...
  const fetchBooks = async () => {
    const token = localStorage.getItem('token');
    if (token === 'demo-token') {
      setBooks([{_id: '1', title: 'The Great Gatsby', author: 'F. Scott Fitzgerald', isbn: '978-0-7432-7356-5', category: 'Fiction', totalCopies: 5, availableCopies: 3}]);
      return;
    }
    try {
...
const Issues = () => {
  ...
  const fetchIssues = async () => {
    const token = localStorage.getItem('token');
    if (token === 'demo-token') {
      setIssues([{_id: '1', bookId: {title: 'The Great Gatsby'}, studentName: 'John Doe', studentId: 'ST001', issueDate: new Date(), dueDate: new Date(Date.now() + 14*24*60*60*1000), status: 'issued'}]);
      return;
    }
    try {
  ...
  const fetchBooks = async () => {
    const token = localStorage.getItem('token');
    if (token === 'demo-token') {
      setBooks([{_id: '1', title: 'The Great Gatsby', availableCopies: 3}]);
      return;
    }
    try {
      const response = await api.get('/books');
...
const Approvals = () => {
  ...
  const fetchApprovals = async () => {
    const token = localStorage.getItem('token');
    if (token === 'demo-token') {
      setApprovals([{_id: '1', title: '1984', author: 'George Orwell', isbn: '978-0-452-28423-4', category: 'Fiction', donorName: 'John Doe', status: 'pending'}]);
      return;
    }
    try {
